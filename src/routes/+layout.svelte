<script>
    ...
    // Self-hosted fonts (only the exact weights/styles actually used below)
    // instead of the Google Fonts CDN: that used to add an external
    // fonts.googleapis.com -> fonts.gstatic.com request chain to the
    // render-blocking critical path (measured ~320ms of pure network
    // latency per font file, on top of the CSS round trip itself). Bundled
    // through Vite instead, the font files ship from the same origin as
    // everything else and are fingerprinted/cached like any other asset.
